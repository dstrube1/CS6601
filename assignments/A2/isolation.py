...
class Board:
    ...
    def set_state(self, board_state, p1_turn=True):
        """
        Function to immediately bring a board to a desired state. Useful for testing purposes;
        call board.play_isolation() afterwards to play
        Parameters:
        ...
        if last_move_q1 != []:
            # set last move to the first found occurrence of 'Q1'
    ...
    def __apply_move__(self, queen_move):
        ...
        if self.move_is_in_board(my_pos[0], my_pos[1]):
            # last position should not be blocked in skid variant
        ...
        # rotate the queens
        self.__active_players_queen__, self.__inactive_players_queen__ = self.__inactive_players_queen__, \
    ...
    def __get_moves__(self, move):
        """
        Get all legal moves of a player on current board state as a list of possible moves.
    ...
    def space_is_open(self, row, col):
        """
        Sanity check to see if a space is within the bounds of the board and blank.
        Not meant to be called directly if you don't know what
        you're looking for.
        Parameters:
    ...
    """    # this function not needed for skid variant - not used
