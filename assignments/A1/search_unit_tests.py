...
class SearchUnitTests(unittest.TestCase):
    ...
    def same_node_bi(self, graph, method, test_count=10, **kwargs):
        """
        Run a bidirectional test search using same start and end node.
        ...
        for src in keys:
            for dst in keys:
                ...
                if src != dst:  # we want path == [] if src == dst
                    self.assertTrue(len(path) == len(ref_path),
                                    msg="Path is too long. Real path: %s, your path: %s" % (ref_path, path))
